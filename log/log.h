...
#define LOG_ASSERT(cond, ...) \
    (!(cond) ? ((void)Alias::log_write_assert(Alias::LogLevel::Level::FATAL, LOG_TAG, __VA_ARGS__)) \
                        : (void)0)
#endif
...
void log_write_assert(int level, const char *tag, const char *fmt, ...) __attribute__((format(printf, 3, 4)));
